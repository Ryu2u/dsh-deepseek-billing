window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 滚轮翻页的冷却：一次滑动往往连发多个事件，不防抖会一滑翻好几页。 */
		 ...
		 * 交互：鼠标停上去暂停自动轮换；在胶囊上滚动滚轮可手动翻页（向上=上一页，向下=下一页）。
		 */
		function RotatingPill() {
			...
			/** 下一张要上场的页从哪个方向滑入（自动轮换时交替，滚轮时跟滚动方向）。 */
			...
			/**
			 * 滚轮翻页：只按方向切到另一页，**不 preventDefault** —— 拦掉默认行为会连带
			 * 锁死页面在输入框上方的滚动。防抖窗口内的重复事件直接忽略。
			 * 向下滚时新页从下方滑入、向上滚时从上方滑入，方向与手感一致。
			 */
			const onWheel = (event) => {
				...
				entryRef.current = delta > 0 ? "down" : "up";
