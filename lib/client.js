window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * 输入框工具行的宽度阈值（px）：工具行窄于它时收起末尾的 token 段，宽回来再放出。
		 *
		 * 工具行放得下整枚胶囊（峰谷 + 余额 + 今日/当月消费 + token）大约需要「胶囊自然宽度
		 * ＋左右两侧按钮」，实测约 900px。宿主给 .uV2eYG_row 声明了
		 * container-type: inline-size，所以样式表里直接用容器查询判定，
		 * 不必猜视口宽度 —— 侧栏开关、窗口缩放都会自动反映到工具行宽度上。
		 */
		const COMPOSER_ROW_HIDE_TOKENS = 900;
		/** 容器查询不可用时的视口回退阈值：视口 ≈ 工具行 + 左右留白约 17px。 */
		const VIEWPORT_HIDE_TOKENS = 920;
		/**
		 * OpenCode 胶囊的收放阈值（工具行宽度，px）：窄于它就把周/月段收起来，只留滚动 5 小时主数字。
		 *
		 * 实测（轮播里两枚各自的自然宽度：DeepSeek 带 token 约 519px、OpenCode 全量约 175px）：
		 * 两者同时在位时单行需要 ≥1330px，窄了就换行；只留主数字后 850px 都还能排一行。
		 * 「重置 …」时刻挂在周/月段里一并收起，悬停明细里始终能查到。
		 */
		...
		const CSS = [
			...
			// 所以这里按「工具行宽度」判定：放不下整枚胶囊时收起末尾的 token 段
			// （余额、今日/当月消费始终保留，token 明细仍在悬停 title 里）。
			// 胶囊回绕到独占一行时行宽必然也小，同样命中这条规则，不会出现"换行了却还硬塞"的中间态。
			...
			".dsb-rotor{position:relative;display:inline-block;height:26px;box-sizing:border-box}",
