window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * 输入框工具行的宽度阈值（px），从宽到窄三档依次让位：
		 *
		 *   ≥900  整枚胶囊（峰谷 + 余额 + 今日/当月消费 + token）
		 *   <900  收 token 段
		 *   <470  再收「今日/当月消费」，只留「谷 梁文谷 距峰时 … │ ¥2.70」约 230px
		 *
		 * 第三档是给手机竖屏的：可用宽度只有 380px 左右，不收这一段胶囊会顶出容器被裁掉。
		...
		const CSS = [
			...
			// 所以这里按「工具行宽度」判定，两档依次让位（明细始终在悬停 title 里）。
			...
			// 手机档：连「今日/当月消费」一起收起，只剩峰谷 + 余额，手机竖屏也放得下。
			...
			// 兜底：极端窄的容器里不让胶囊顶出容器（正常手机上第三档已经生效，用不到这条）。
