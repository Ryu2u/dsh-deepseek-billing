window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			".dsb-rotor{position:relative;display:inline-block;height:26px;box-sizing:border-box;min-width:0;cursor:grab;touch-action:none;user-select:none;-webkit-user-select:none}",
			// 拖拽中：光标变抓手，且停掉过渡 —— 否则页跟着过渡曲线走，手感发飘。
			".dsb-rotor-dragging{cursor:grabbing}",
			".dsb-rotor-dragging .dsb-rotor-item{transition:none}",
		...
		/** 光标上下拖动多少像素才算翻页：太小会跟「点一下」混淆，太大会拖不动。 */
		const DRAG_SWITCH_PX = 16;
		 ...
		 * 交互：鼠标停上去暂停自动轮换；**按住卡片上下拖动**可手动翻页
		 * （向上拖=下一张从下方滑入，向下拖=上一张从上方滑入）。
		 */
		function RotatingPill() {
			...
			/** 拖拽起点（仅在按下期间有效）：null 表示当前没有拖拽。 */
			const dragRef = React.useRef(null);
			/** 下一张要上场的页从哪个方向滑入（自动轮换时交替，拖拽时跟拖动方向）。 */
			const entryRef = React.useRef("down");

			React.useEffect(() => {
				// 拖动过程中不要自动翻页，否则手还在拖就被定时器抢走。
				if (paused || dragging) return undefined;
				const timer = setInterval(() => {
					// 自动轮换时进入方向交替，形成「一去一回」的翻卡片感。
					setIndex((current) => {
						entryRef.current = current === 0 ? "down" : "up";
			...
			/**
			 * 移动：沿竖直方向拖动超过阈值就翻一页并结束本次拖拽（松手前不重复翻）。
			 * 判定只认竖直分量（在胶囊上左右拖多半是想选中文字，不该翻页）。
			 */
			const onPointerMove = (event) => {
				...
				if (Math.abs(dy) < DRAG_SWITCH_PX || Math.abs(dy) < Math.abs(dx)) return;
				dragRef.current = null;
				setDragging(false);
				// 向上拖 = 下一张从下方滑入；向下拖 = 上一张从上方滑入。
				entryRef.current = dy < 0 ? "down" : "up";
			...
			const item = (key, position, child) => React.createElement("span", {
				...
				// 两页同向位移：data-from=down 表示「从下方滑入 / 向上移出」，
				// 所以离场页与入场页用同一个方向值，才是「整叠往上/往下走」的手感。
				"data-from": position === index ? "none" : entryRef.current,
			...
			return React.createElement("div", {
				...
				onMouseLeave: () => {
					setPaused(false);
					// 鼠标拖到外面再松开时 pointerup 不一定落在本元素上，这里补一次收尾。
					endDrag();
				},
				// 拖拽用指针事件：鼠标、触摸、笔走同一套。
				onPointerDown,
