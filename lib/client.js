window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			// box-sizing:border-box 是给轮播格子用的：格子高 26px，胶囊得正好 26px
			// 才不会被裁掉上下那 1px 边框（宿主若没做全局 border-box，这里兜住）。
			".dsb-pill{display:inline-flex;align-items:center;gap:7px;box-sizing:border-box;height:26px;padding:0 10px 0 8px;border:1px solid var(--dsw-alias-border-l1);border-radius:999px;background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);font-size:12px;line-height:1;white-space:nowrap;user-select:none;cursor:default}",
			...
			// 竖向轮播：两页绝对定位叠在同一处，靠 translateY 上下走。
			// 宽度由脚本跟着「当前那页」写回（见 RotatingPill）：叠放与动画交给 CSS。
			...
			// 所以测量与写回都只针对当前页（见 RotatingPill 里的 measure）。
			//
			// overflow:hidden = 「卡片格子」：格子外的部分一律不画。高度与胶囊同为 26px；
			// 横向也裁 —— 两页宽度差得远（DSB 约 513px、OC 约 175px），不裁的话宽的那页
			// 滑进来时会横着盖住旁边的界面。翻页落地后格子再跟着新页的宽度变。
			".dsb-rotor{position:relative;display:inline-block;height:26px;box-sizing:border-box;min-width:0;overflow:hidden;cursor:grab;touch-action:none;user-select:none;-webkit-user-select:none}",
			// 两页共用一条位移：--dsb-drag（px）由脚本在拖动时实时写进来，每页的 transform 都是
			// 「自己该停的位置 + 这条位移」，所以拖动时两页是**一整叠跟着手走**，不是各自跳。
			".dsb-rotor-item{position:absolute;left:0;top:0;display:inline-flex;min-width:0;transition:transform 260ms ease,opacity 260ms ease;transform:translateY(var(--dsb-drag,0px))}",
			// 轮播里的胶囊不设 max-width：让它始终按自然宽度上报（否则会测到被压窄的值）。
			".dsb-rotor-item>.dsb-pill{flex:none;max-width:none}",
			// 停在格子外的那页：隐形。visibility 延迟到淡出跑完再收，离场时才不会「啪」地消失。
			".dsb-rotor-item[data-slot=off]{opacity:0;pointer-events:none;visibility:hidden;transition:transform 260ms ease,opacity 260ms ease,visibility 0s linear 260ms}",
			// 停靠位置：100% 正好是「贴着卡片上/下沿」，翻页时两页像连续的一叠一起走
			// （用 120% 会在两页之间留一条 5px 的缝，拖到一半时能看见那条缝）。
			".dsb-rotor-item[data-from=down]{transform:translateY(calc(100% + var(--dsb-drag,0px)))}",
			".dsb-rotor-item[data-from=up]{transform:translateY(calc(-100% + var(--dsb-drag,0px)))}",
			// 拖动中：光标变抓手、停掉过渡（否则页跟着过渡曲线走，手感发飘），
			// 并让停在格子外的那页显形 —— 它得跟着手指滑进来，才有「拖得动」的感觉。
			// 选择器故意写长一点，压过上面 [data-slot=off] 的 transition/visibility。
			".dsb-rotor.dsb-rotor-dragging{cursor:grabbing}",
			".dsb-rotor.dsb-rotor-dragging .dsb-rotor-item{transition:none}",
			".dsb-rotor.dsb-rotor-dragging .dsb-rotor-item[data-slot=off]{opacity:1;visibility:visible}",
		...
		/** 拖动超过多少像素才「拖到位」：约半张卡片（26px），松手时超过它才翻页，否则回弹。 */
		const DRAG_SWITCH_PX = 12;
		/** 超过这个距离后位移开始带阻尼（橡皮筋），避免越拖越离谱。 */
		const DRAG_RUBBER_PX = 22;
		/** 阻尼系数：越拖越沉。 */
		const DRAG_RUBBER_RATE = 0.45;
		/** 位移上限：橡皮筋再沉也不让另一页被拖过头（约卡片高的 1.3 倍）。 */
		const DRAG_MAX_PX = 34;
		/** 停靠方向的对面：往下拖进来的那张，离场时正是朝上走。 */
		const otherSide = (side) => (side === "down" ? "up" : "down");

		/**
		 * 竖向轮播座位：同一个位置轮流显示 DeepSeek 与 OpenCode Go 两枚胶囊。
		 *
		 * 两页常驻渲染（各自独立轮询自己的路由），当前那页居中，另一页停在格子外面（上下各一侧）。
		 * 容器宽度跟着「当前那页」的实测宽度走，所以每页都是自己的自然宽度、不会被另一页挤窄。
		 *
		 * 交互：
		 *   - 自动轮换：每 5 秒翻一页，鼠标停上去暂停；
		 *   - 按住卡片上下拖动：两页**跟着光标实时位移**（位移写进 --dsb-drag，拖动中停掉过渡），
		 *     拖过半张卡片松手就翻页、否则回弹 —— 跟手机滑卡片一个手感。
		 *
		 * 翻页方向：翻的时候「进来的那页从哪一侧来」= sideRef；那页进来、另一页就朝对面走，
		 * 于是它下一次正好从对面进来 —— 整叠像一张牌那样上下交替，自动轮换与拖动共用这一条规则。
		 */
		function RotatingPill() {
			const [index, setIndex] = React.useState(0);
			const [paused, setPaused] = React.useState(false);
			const [dragging, setDragging] = React.useState(false);
			/** 拖动位移（px）：正=往下拖，负=往上拖；只在拖拽期间非 0。 */
			const [dragOffset, setDragOffset] = React.useState(0);
			// 宽度回调在观察器里异步触发，用 ref 读当前是哪一页，避免闭包读到旧值。
			const indexRef = React.useRef(0);
			indexRef.current = index;
			/**
			 * 拖拽会话（null = 没在拖）：起点坐标 + 实时竖直位移（原始值，不带阻尼）。
			 * 位移记在 ref 里是为了松手时读到最新值 —— 状态更新是异步的，
			 * 同一帧里先 move 再 up 的话，闭包里的 dragOffset 会慢一拍。
			 */
			const dragRef = React.useRef(null);
			/**
			 * 另一页停在哪一侧（也就是它下次从哪一侧滑进来）：
			 * 自动轮换每翻一页就换到对面，拖动时跟着手势方向走（往上拖 = 它从下方来）。
			 */
			const sideRef = React.useRef("down");

			React.useEffect(() => {
				// 拖拽期间不自动翻页，否则手还在拖就被定时器抢走。
				if (paused || dragging) return undefined;
				const timer = setInterval(() => {
					// 换到对面：这一页进来的方向，就是下一页进来的方向的反面。
					sideRef.current = otherSide(sideRef.current);
					setIndex((current) => (current === 0 ? 1 : 0));
				}, ROTATE_HOLD_MS);
				return () => clearInterval(timer);
			}, [paused, dragging]);

			/** 按下：开一次拖拽会话（同时暂停自动轮换），并抓住指针。 */
			const onPointerDown = (event) => {
				// 只响应主键（左键/触摸/笔），避免右键菜单也触发。
				if (event.button !== undefined && event.button !== 0) return;
				const node = event.currentTarget;
				const session = { x: event.clientX, y: event.clientY, dy: 0, captured: false };
				dragRef.current = session;
				setDragging(true);
				setDragOffset(0);
				// 指针捕获：格子只有 26px 高，手一抖就出界；不抓的话 move/up 全丢在外面，
				// 一次完整的拖动会被拆成「拖一下 → 松手没收到 → 悬着」，手感就成了随机翻页。
				if (node !== null && event.pointerId !== undefined && typeof node.setPointerCapture === "function") {
					try {
						node.setPointerCapture(event.pointerId);
						session.captured = true;
					} catch (error) {
						// 捕获失败不影响主流程：还有 pointerup / pointercancel 兜底。
					}
				}
				// 拖的时候别把胶囊里的文字/数值选中。
			...
			/**
			 * 移动：两页跟着光标实时位移（整叠一起走），另一页预先停到手势来的那一侧。
			 * 以水平为主的拖动直接忽略 —— 在卡片上左右拖多半是想选中文字，不该翻页。
			 */
			const onPointerMove = (event) => {
				...
				session.dy = dy;
				// 手势方向 = 另一页该待的那一侧：往上拖，下一张就该从下方滑进来。
				// 换边只发生在过零那一刻 —— 那会儿它正好卡在格子边缘，看不见跳变。
				const side = dy < 0 ? "down" : "up";
				if (sideRef.current !== side) sideRef.current = side;
				const magnitude = Math.abs(dy);
				const damped = magnitude <= DRAG_RUBBER_PX
					? magnitude
					: Math.min(DRAG_MAX_PX, DRAG_RUBBER_PX + (magnitude - DRAG_RUBBER_PX) * DRAG_RUBBER_RATE);
				setDragOffset(dy < 0 ? -damped : damped);
			};

			/**
			 * 松手：拖过半张卡片就翻页（离场那页朝拖动方向走、停到对面），否则回弹。
			 * pointerup / pointercancel / 丢失捕获 都走这里，重复调用是安全的。
			 */
			const endDrag = () => {
				const session = dragRef.current;
				if (session === null) return;
				...
				setDragging(false);
				setDragOffset(0);
				if (Math.abs(session.dy) < DRAG_SWITCH_PX) return;
				// 这一页朝拖动方向离场、停在对面：下一轮它就从那边进来。
				sideRef.current = otherSide(sideRef.current);
				setIndex((current) => (current === 0 ? 1 : 0));
			};

			const [width, setWidth] = React.useState(0);
			/** 两页的 DOM 节点（ref 回调挂进来）：切页后要重新量，所以得留一份。 */
			const nodes = React.useRef([null, null]);
			/**
			 * 量「当前那页」的自然宽度，写回格子宽度。
			 * 只认当前页：非当前页会被格子挤窄，把压窄后的值写回去会把容器带偏（这个 bug 真实出现过）。
			 */
			const measure = () => {
				const node = nodes.current[indexRef.current];
				if (node === null || node === undefined) return;
				const measured = Math.ceil(node.getBoundingClientRect().width);
				if (measured > 0) setWidth(measured);
			};
			/**
			 * 切页后必须主动重量一次，别指望 ResizeObserver：格子变宽变窄**不会**改变页自身的尺寸，
			 * 观察器不会响（实测格子 175px→513px 时页的尺寸纹丝不动、RO 一次回调都没有）。
			 * 而格子是 overflow:hidden —— 宽度不跟过去，新那页就会被裁掉半截。
			 * 用 layout effect：跟 ref 回调一样在浏览器绘制前跑完，
			 * 否则首帧宽度还是 0（格子是空的），胶囊会闪一下才出来。
			 */
			React.useLayoutEffect(measure, [index]);
			...
			const item = (key, position, child) => React.createElement("span", {
				...
				// data-from = 这一页停在哪一侧（也是它下次从哪一侧滑进来）：
				// 「down」= 停在下方，所以它进来时整叠往上走、另一页朝上离场。
				"data-from": position === index ? "none" : sideRef.current,
				"aria-hidden": position === index ? undefined : "true",
			}, child);

			const style = {};
			if (width > 0) style.width = width + "px";
			// 拖动位移走 CSS 变量：每页的 transform 是「停靠位置 + 这条位移」在样式表里相加的，
			// 免得把百分比与像素的加法在 JS 里再写一遍、两边还可能不一致。
			if (dragOffset !== 0) style["--dsb-drag"] = dragOffset + "px";

			return React.createElement("div", {
				className: dragging ? "dsb-rotor dsb-rotor-dragging" : "dsb-rotor",
				style,
				...
				onMouseLeave: () => {
					setPaused(false);
					// 指针捕获失败时（捕获不可用），光标拖出格子就收不到 up 了，这里补一次收尾。
					// 捕获成功时 move/up 都会回到本元素上，不用在离开时打断拖动。
					const session = dragRef.current;
					if (session !== null && session.captured !== true) endDrag();
				},
				// 拖拽用指针事件：鼠标、触摸、笔走同一套。
				onPointerDown,
				onPointerMove,
				...
				// 捕获意外丢失（例如系统接管了指针）时收尾，避免一直卡在拖拽态不自动翻页。
