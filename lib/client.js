window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			// 不再用脚本量宽度 —— 之前那套「量当前页宽度 → 写回容器宽度」会形成反馈环：
			// 非当前页缩窄后上报的宽度变小，把容器带偏，连带内容查询判定错、空间白白空着。
			// 当前页居中；要上场的那页从下（或上）滑入，离场的原地淡出。
			".dsb-rotor{display:inline-grid;grid-template-columns:minmax(0,max-content);height:26px;box-sizing:border-box;min-width:0}",
			".dsb-rotor-item{grid-area:1/1;display:inline-flex;min-width:0;transition:transform 260ms ease,opacity 260ms ease}",
			// 轮播里的胶囊不设 max-width：让它始终按自然宽度上报（否则宽度反馈环会回来）。
		 ...
		 * **宽度完全交给 CSS**：两页叠在同一个网格单元里，单元宽度取较宽的那页，
		 * 于是每页都按自然宽度显示，也不需要脚本量宽度、更不会形成宽度反馈环。
