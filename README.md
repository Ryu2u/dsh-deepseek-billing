...
- DeepSeek 那枚按工具行宽度分四档，从宽到窄依次让位：
  ...
  | 工具行宽度 | 显示内容 |
  |---|---|
  | ≥1500px | 整枚：`[峰] 梁文锋 距谷时 … │ ¥12.48 │ 今日 … · 当月 … │ 今 … · 月 … tok` |
  | <1500px | 收起 token 段 |
  | <470px | 再收起「今日/当月消费」，剩 `[谷] 梁文谷 距峰时 … │ ¥2.70`（约 229px） |
  | <450px | **手机上只剩** `[谷] ¥2.70`（约 75px） |

  token 档定得高（1500）是因为行内还有左右两组工具（约 890px）：整枚胶囊约 600~700px，
  加内边距合计约 1590px，token 段只在很宽的窗口才排得下 —— 宁可少显示一段，
  也不让两侧按钮跟着换行。后两档是给手机竖屏的（可用宽度约 380px）。
  第四档保留峰谷汉字与余额，这枚胶囊最核心的两个信息。

- OpenCode 那枚：工具行窄于 **1330px** 收起「周 · 月」，只剩滚动 5 小时主数字。
  但窄于 **450px**（手机档）反而把「周 · 月」放回来 —— 这一档 DeepSeek 那枚已只剩 `[谷] ¥2.70`，
  位置腾出来了，而 OpenCode 全量才约 175px，放得下。
...
> **轮播宽度交给 CSS，不要改回脚本量宽度。** 两页叠在同一个网格单元里（`grid-area:1/1`），
> 单元宽度取较宽的那页，所以每页都按自然宽度显示。曾经用「ResizeObserver 量当前页宽度 →
> 写回容器宽度」实现，会形成反馈环：非当前页被挤窄后以偏小的宽度上报，把容器带偏、
> 内容查询判定跟着错，页面上就会空出一大块（见 `test/client-render.test.mjs` 里那条守它的用例）。
