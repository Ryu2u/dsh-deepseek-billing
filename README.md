...
在会话头部右侧工具区和输入框工具行各有一个**轮播位** —— 同一个位置每 5 秒竖向翻页，
...
- **自动轮换**：每 5 秒翻一页，进入方向交替，形成「一去一回」的翻卡片感；
- **按住卡片上下拖动**：两页**跟着光标实时位移**（像手机滑卡片那样跟手），
  拖过半张卡片（12px）松手就翻页、没拖到位松手回弹；向上拖 = 下一张从下方滑入，
  向下拖 = 上一张从上方滑入；以水平为主的拖动不跟手也不翻页（在卡片上左右拖多半是想
  选文字）；按下时抓住指针，所以拖到卡片外面也不会断；没按下就移动鼠标不翻页；
- **鼠标停上去自动暂停**，方便看数字和悬停明细。

轮播容器是「卡片格子」（`overflow:hidden`，高 26px 与胶囊同高）：格子外的页一律不画，
两页宽度差得远（DeepSeek 约 500px、OpenCode 约 175px）也不会互相盖住旁边的界面。
容器宽度跟着当前那枚走，所以每枚都是自己的自然宽度，不会被另一枚挤窄。
...
两枚胶囊都会随输入框工具行的宽度自动收放。判定用的是**宿主工具行的宽度**
（宿主给 `.uV2eYG_row` 声明了 `container-type: inline-size`），所以拖窗口、开关侧栏都即时生效。
从宽到窄依次让位（下表阈值为工具行宽度）：

| 工具行宽度 | DeepSeek 那页 | OpenCode 那页 |
|---|---|---|
| ≥1050px | 整枚：`[峰] 梁文锋 距谷时 … │ ¥12.48 │ 今日 … · 当月 … │ 今 … · 月 … tok`（约 513px） | `OC Go 2% │ 周 7% · 月 3%`（约 175px） |
| <1020px | 收起 token 段 | 收起「周 · 月」，只剩主数字 |
| <1000px | 同上（约 367px） | 只剩主数字（约 89px） |
| <900px | 再收起「今日/当月消费」（约 229px） | 同上 |
| <820px | **手机上只剩** `[谷] ¥2.70`（约 75px） | 同上 |

阈值都在 `lib/client.js` 顶部：`ROW_HIDE_TOKENS_BELOW` / `ROW_HIDE_STATS_BELOW` /
`ROW_HIDE_TEXT_BELOW` / `OPENCODE_HIDE_DETAIL_BELOW`，外加一组视口回退阈值
（只在不支持容器查询的老引擎上生效）。
1000 这个数是实测的：完整行（左右两组工具 + 整枚胶囊 513px）在行宽 950px 仍单行、900px 才换行。

> ### 改这块前先读：两条走不通的路 + 一条必须做对的
>
> 1. **不要把 `container-type` 挂到胶囊或轮播页壳上，改成「按胶囊自身宽度查询」。**
>    尺寸容器会按其自身尺寸计算内容——规范上该元素的尺寸贡献被清零，实测轮播容器直接塌成 0 宽
>    （四种挂载点全部试过，无一例外）。
> 2. **也不要为了「自适应内容」回到「脚本量宽度 → 写回容器宽度」。**
>    两页宽度不同，非当前页会被压窄，把压窄后的值写回去会把容器带偏，
>    连带分档判定错、页面上空出一大块（这个 bug 真实出现过）。
>
> 现在用的是：宽度由脚本跟着**当前页**写回（只写当前页，实测反复切页不漂移），
> 分档判定一律用宿主行宽。`test/client-render.test.mjs` 有两条用例守着这两条结论。
>
> 3. **拖动一定要 `setPointerCapture`。** 格子只有 26px 高，光标一离开它就再也收不到
>    `pointermove`/`pointerup`，一次完整的拖动会被拆成「拖一下 → 松手没收到 → 悬着」，
>    手感就是随机翻页。实测对照：抓指针的那组出界 60px 后仍收到 3 个 move + 1 个 up，
>    不抓的那组收到 0 个。

**收起来的明细都在悬停（title）里，一样都不少**：峰谷规则、当前时段、余额构成（充值/赠金）、
今日与当月消费、今日与当月 token、下次切换的具体时刻与倒计时、数据更新时间。
OpenCode 那页的悬停里给出三个窗口的百分比、各自的重置时刻、数据更新时间与实际请求的接口地址。
...
| `test/client-render.test.mjs` | 客户端半边：在最小 React 运行时里真跑一遍 `apply()` + 渲染，抓未定义标识符、座位注册、轮播页数、数据到手后的渲染、样式表阈值、拖动状态机（跟手位移 / 松手才翻页 / 回弹 / 指针捕获）与「切页后容器宽度跟着新页走」 |
